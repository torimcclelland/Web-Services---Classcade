...
const ZoomCallback = () => {
  ...
  useEffect(() => {
    ...
    const handleCallback = async () => {
      ...
      const userId = searchParams.get("state") || searchParams.get("userId");

      if (connected === "true") {
        setStatus("Successfully connected! Redirecting...");
        setTimeout(() => navigate("/zoom?connected=true"), 800);
        return;
      }

      if (error) {
        setStatus("Authorization failed. Redirecting...");
        setTimeout(() => navigate("/zoom?error=auth_failed"), 1500);
        return;
      }

      if (!code || !userId) {
        setStatus("Invalid callback parameters. Redirecting...");
        setTimeout(() => navigate("/zoom?error=invalid_params"), 1500);
        return;
      }
      ...
      try {
        ...
        const response = await fetch(
          "http://localhost:4000/api/zoom/callback",
          {
            method: "POST",
