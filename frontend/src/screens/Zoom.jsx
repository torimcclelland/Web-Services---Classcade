...
const Zoom = () => {
  ...
  const checkZoomConnection = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4000/api/zoom/meetings?userId=${userId}`
      );
      if (response.ok) {
        setIsConnected(true);
  ...
  const fetchTeamMeeting = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4000/api/zoom/team-meeting/${projectId}`
      );
      const data = await response.json();
  ...
  useEffect(() => {
    ...
    if (projectId) {
      ...
      newSocket.on(
        "teamMeetingUpdate",
        ({ teamMeeting: updatedTeamMeeting }) => {
  ...
  const handleConnectZoom = async () => {
    if (!userId) return;
    window.location.href = `http://localhost:4000/api/zoom/auth?userId=${userId}`;
  };
  ...
  const handleCreateMeeting = async () => {
    ...
    try {
      ...
      if (response.ok && data.meeting?.join_url) {
        ...
        alert(
          `Failed to create Zoom meeting: ${data.error || "Unknown error"}`
        );
      }
    } catch (error) {
  ...
  const handleStartTeamMeeting = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:4000/api/zoom/team-meeting",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (response.ok && data.teamMeeting) {
        setTeamMeeting(data.teamMeeting);
        socket?.emit("teamMeetingCreated", {
          projectId,
      ...
      alert(
        "Something went wrong while starting the team meeting. Please try again."
      );
    } finally {
      setLoading(false);
    }
        ...
        <div style={ZoomStyle.container}>
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  >
                    ...
                    >
                      Started by {teamMeeting.createdBy?.firstName || "Unknown"}{" "}
                      {teamMeeting.createdBy?.lastName || ""}
                    </span>
                  </div>
                </div>
          ...
          {meetings.length > 0 ? (
            <>
              ...
              >
                <h2 style={{ ...ZoomStyle.sectionTitle, margin: 0 }}>
                  Upcoming Meetings
                </h2>
                <button
              ...
              <div style={ZoomStyle.meetingsGrid}>
                {meetings.map((meeting) => {
                  ...
                  return (
                    ...
                    >
                      <div id={titleId} style={ZoomStyle.meetingTitle}>
                        {meeting.topic}
                      </div>
        ...
        {showEndMeetingModal && (
          <ModalWrapper onClose={() => setShowEndMeetingModal(false)}>
            <div style={{ textAlign: "center" }}>
              ...
              >
                This will end the meeting for all team members. This action
                cannot be undone.
              </p>
              <div
                style={{
        ...
        {showDisconnectModal && (
          <ModalWrapper onClose={() => setShowDisconnectModal(false)}>
            <div style={{ textAlign: "center" }}>
              ...
              >
                You will need to reconnect your Zoom account to create or join
                meetings again.
              </p>
              <div
                style={{
