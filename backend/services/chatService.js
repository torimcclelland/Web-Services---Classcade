...
// Create a message (now with optional attachments)
router.post('/', async (req, res) => {
    const { conversationId, channelId, sender, recipients, content, contentType, repliedTo, attachments } = req.body;
    ...
    try {
        ...
        const populatedMsg = await Chat.findById(newMsg._id)
...
router.get('/', async (req, res) => {
    try {
        const { conversationId, channelId, userId, limit = 50, before } = req.query;
        const q = {};
